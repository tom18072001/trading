...
Importing this module is inert. It used to build the whole report — and send
the email — as a side effect of `import`, which is why it had zero tests and why
`/api/state/report/send` shells out to a subprocess (§24.3, §20.3 P3-2). All
1,500 lines of work now sit inside `main()` behind an `if __name__` guard. The
pure pieces live in `services/report/` and are importable on their own.
"""
import os
import sys
import sqlite3
...
from services.report.charts import (
    make_correlation_heatmap,
    make_mini_chart,
    make_sector_aggregate_chart,
    make_sector_flow_chart,
)
from services.report.data import (
    latest_flow_daily,
    latest_macro,
    latest_regime,
    latest_signals,
...
def main(argv: list[str] | None = None) -> None:
    ...
    Everything below used to run at import time. It is one function now, not
    because it wants to be one — 1,500 lines is not a function — but because an
    `if __name__` guard was the only change that made `import generate_report`
    safe, and safe-to-import is what unblocks a test and what `/api/state/report/
    send` needs before it can stop shelling out (§24.3).

    Splitting the weave itself is a rewrite: ~700 lines of `X = build_x()` where
    each builder reads several of the others' locals. `ponytail:` — do it when a
    second consumer needs the pieces, not before.
    """
    ...
    # Style, chart helpers and the two formatters moved to services/report/.
    # `open_db` keeps the temp-copy fallback for WAL on a network mount
    # (§18.4/18) — see services/report/data.py.
    con = open_db(DB_PATH)
    ...
    sector_recent_dv   = defaultdict(float)
    ...
    # SQL reads moved to services/report/data.py; they take the cursor as an
    # argument now instead of closing over it, which is what made them testable.
    regime   = latest_regime(cur)
    macro    = latest_macro(cur)
    signals  = latest_signals(cur)
    flow_d   = latest_flow_daily(cur)
    code2name = sector_name_map(cur)
    ...
    # Chart builders moved to services/report/charts.py.
